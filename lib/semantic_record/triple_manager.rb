...
module TripleManager
  ...
  def self.property_for(s,p)
    ...
    # 
    # 
    #     return predicate.get_property("http://www.w3.org/TR/rdf-schema#range")
  ...
  def self.populate_model_with_result_from_http(uri)
    ...
    puts "get from remote: #{@@transit_model.size}"
    ...
    puts "got from remote: #{@@transit_model.size}"
  ...
  def self.get_subjects(s)
    ...
    get_by_sparql(query_string,true)     
  end
